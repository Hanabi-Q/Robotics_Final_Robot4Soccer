...
class Simulation:
    ...
    def __init__(self, player, ball, shockable, full_vision):
        ...
        self.goal_red_time = datetime.datetime.utcnow()
        self.goal_yellow_time = datetime.datetime.utcnow()
    ...
    def check_goal(self):
        ...
        if (round(ball_position.x + RADIUS_BALL) <= 30 and
                round(ball_position.y - RADIUS_BALL) >= (round(SCREEN_HEIGHT) / 2 - 100) and
                round(ball_position.y + RADIUS_BALL) <= (round(SCREEN_HEIGHT) / 2 + 100)):
            if (now - self.goal_red_time).total_seconds() > 1:
                self.right_goal += 1
                self.goal_red_time = now
                just_scored = True

        if (round(ball_position.x - RADIUS_BALL) >= (round(SCREEN_WIDTH) - 30) and
                round(ball_position.y - RADIUS_BALL) >= (round(SCREEN_HEIGHT) / 2 - 100) and
                round(ball_position.y + RADIUS_BALL) <= (round(SCREEN_HEIGHT) / 2 + 100)):
            if (now - self.goal_yellow_time).total_seconds() > 1:
                self.left_goal += 1
                self.goal_yellow_time = now
                just_scored = True

        if just_scored:
            #print("[INFO] Goal detected. Restarting match...")
            self.restart_game()
    '''
    ...
    def restart_game(self):
        ...
        self.ball.pose.position.x = SCREEN_WIDTH * PIX2M * 1 / 4
        self.ball.pose.position.y = SCREEN_HEIGHT * PIX2M / 2
        self.ball.pose.rotation = 0
        self.ball.linear_speed = 0.0
        self.ball.angular_speed = 0.0

        
        for i in range(len(self.player)):
            self.player[i].pose = deepcopy(self.initial_position[i + 1])  
            self.player[i].linear_speed = 0.0
            self.player[i].angular_speed = 0.0
