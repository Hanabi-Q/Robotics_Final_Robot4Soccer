...
class Player(Agent):
    ...
    def __init__(self, pose, max_linear_speed, max_angular_speed, radius):
        ...
        self.init_x = pose.position.x
        self.init_y = pose.position.y
        self.init_rot = pose.rotation

    def reset(self):
        self.pose.position.x = self.init_x
        self.pose.position.y = self.init_y
        self.pose.rotation = self.init_rot
        self.linear_speed = 0
...
class Ball(Agent):
    ...
    def reset(self):
        self.pose = Pose(SCREEN_WIDTH * PIX2M / 2, SCREEN_HEIGHT * PIX2M / 2, 0)
        self.linear_speed = 0
        self.angular_speed = 0
