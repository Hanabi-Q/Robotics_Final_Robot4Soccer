...
class DiscreteSACAgent:
    def __init__(self, state_dim, action_dim):
        ...
        self.memory = type('', (), {'push': self.push})()
        self.action_dim = action_dim
        self.update = lambda: self.train(batch_size=256)

    def select_action(self, state):
        state_tensor = torch.FloatTensor(np.array(state).reshape(1, -1)).to(self.device)
    ...
    def train(self, batch_size=256):
        ...
        with torch.no_grad():
            ...
            next_actions = torch.eye(self.action_dim, device=self.device).unsqueeze(0).repeat(batch_size, 1, 1)
            next_states_repeat = next_states.unsqueeze(1).repeat(1, self.action_dim, 1)

            q1, q2 = self.critic_target(
                next_states_repeat.view(-1, states.shape[1]),
                next_actions.view(-1, self.action_dim)
            )
            q_min = torch.min(q1, q2).view(batch_size, self.action_dim)

            target_q = (probs * (q_min - torch.exp(self.log_alpha) * log_probs)).sum(dim=1, keepdim=True)
            target = rewards + (1 - dones) * self.gamma * target_q

        current_q1, current_q2 = self.critic(states, action_onehot)
        ...
        for a in range(self.action_dim):
            onehot = torch.eye(self.action_dim)[a].to(self.device).unsqueeze(0).repeat(batch_size, 1)
            q1a, q2a = self.critic(states, onehot)
            q1_all.append(q1a)
            q2_all.append(q2a)
        ...
        actor_loss = (torch.exp(self.log_alpha) * log_probs - q_min) * probs
        ...
        with torch.no_grad():
            logits_detach = self.actor(states)
            dist_detach = Categorical(logits=logits_detach)
            log_probs_detach = dist_detach.logits - torch.logsumexp(dist_detach.logits, dim=1, keepdim=True)
            entropy = -log_probs_detach.sum(dim=1, keepdim=True)

        target_entropy = -self.action_dim * 0.98
        alpha_loss = -(self.log_alpha * (entropy + target_entropy)).mean()
...
def get_rl_command(action, player, ball, team, simulation):
    ...
    if action == 0:
        ...
        if is_controlling_ball(player, ball):
            ...
            ball.pose.position.x = player.pose.position.x + offset * math.cos(player.pose.rotation)
            ball.pose.position.y = player.pose.position.y + offset * math.sin(player.pose.rotation)
        dribble_angle = clamp_angle(compute_angle_diff(player.pose.rotation, angle_to_ball + math.pi / 6),
                                    DRIBBLE_ANGLE_THRESHOLD)
        return (DRIBBLE_SPEED, dribble_angle)
...
def main():
    ...
    pygame.display.set_caption("Hybrid Control: DQN vs SAC Soccer")
    ...
    agents = [
        ...
        DQNAgent(STATE_SIZE, ACTION_SIZE, double_dqn=False),
        DiscreteSACAgent(STATE_SIZE, ACTION_SIZE),
        DiscreteSACAgent(STATE_SIZE, ACTION_SIZE)
    ]
    ...
    for i, agent in enumerate(agents):
        if hasattr(agent, 'policy_net'):
            torch.save(agent.policy_net.state_dict(), f"agent_{i}_DQN.pth")
        else:
            torch.save(agent.actor.state_dict(), f"agent_{i}_SAC_actor.pth")
            torch.save(agent.critic.state_dict(), f"agent_{i}_SAC_critic.pth")
    ...
    plt.figure(figsize=(8, 5))
    plt.plot(moving_average(reward_history_yellow1), label="Yellow1 (SAC)", linestyle='--')
    plt.plot(moving_average(reward_history_yellow2), label="Yellow2 (SAC)", linestyle='--')
    plt.title("SAC Agents (Yellow Team)")
    plt.xlabel("Episode")
    plt.ylabel("Total Reward")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("SAC_rewards_with_MA.png")
    plt.show()
