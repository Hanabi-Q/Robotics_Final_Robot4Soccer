...
class DuelingDQNAgent(DQNAgent):
    def __init__(self, state_size, action_size):
        super().__init__(state_size, action_size, double_dqn=True)
...
def main():
    ...
    pygame.display.set_caption("Hybrid Control: DQN vs Dueling DQN Soccer")
    ...
    for i, agent in enumerate(agents):
        model_type = "DuelingDQN" if isinstance(agent, DuelingDQNAgent) else "DQN"
        torch.save(agent.policy_net.state_dict(), f"agent_{i}_{model_type}.pth")
    ...
    plt.figure(figsize=(8, 5))
    plt.plot(moving_average(reward_history_yellow1), label="Yellow1 (DuelingDQN)", linestyle='--')
    plt.plot(moving_average(reward_history_yellow2), label="Yellow2 (DuelingDQN)", linestyle='--')
    plt.title("Dueling DQN Agents (Yellow Team)")
    plt.xlabel("Episode")
    plt.ylabel("Total Reward")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("Dueling_DQN_rewards_with_MA.png")
    plt.show()
